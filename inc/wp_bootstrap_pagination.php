...
function gym_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'gym' ),
        'next_string'     => esc_html__( '&raquo;', 'gym' ),
        'before_output'   => '<div class="gym-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'gym_pagination_defaults', $defaults )
    );
